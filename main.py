...
def generate_pipes(count, pipe_width=140, gap=280, min_height=50, max_neight=440, distance=650):
    pipes = []
    start_x = window_size[0]
    for i in range(count):
        height = randint(min_height, max_neight)
...
while sd.InputStream(samplerate=sr, channels=1, blocksize=block, callback=audio_cb):
    while True:
        for e in event.get():
            if e.type == QUIT:
                quit()
        ...
        draw.rect(window, 'green', player_rect)
        for pie in pies[:]:
            if not lose:
                pie.x -= 10
            draw.rect(window, 'Navy', pie)
            if pie.x <= -100:
                pies.remove(pie)
                score+=0.5
